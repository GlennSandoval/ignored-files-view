...
function assertLabelSeq(items, expected) {
  ...
  assert.deepStrictEqual(labels, expected, `labels mismatch: ${labels.join(', ')} vs ${expected.join(', ')}`);
}
...
withMocks(({ registered, vscode, config, gitStub }) => {
  ...
  return Promise.resolve()
    ...
    .then(() => { config.ignoredMaxItems = 123; gitStub._result = { files: ['x.txt'], truncated: true }; provider.refresh(); })
    .then(() => provider.getChildren())
    .then((items2) => {
      assert(items2.length >= 1, 'should include a note item');
      assert(/Showing first 123 ignored files/.test(String(items2[0].label)), 'truncation note shows configured cap');
    ...
    .then(() => { config.ignoredMaxItems = 999999; gitStub._result = { files: [], truncated: false }; provider.refresh(); })
    .then(() => provider.getChildren())
    .then(() => {
    ...
    .then(() => { config.trustEnabled = true; vscode.workspace.isTrusted = false; provider.refresh(); })
    .then(() => provider.getChildren())
    .then((items3) => {
